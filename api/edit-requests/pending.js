...
export default async function handler(req, res) {
  ...
  try {
    ...
    const response = await fetch(
      `${supabaseUrl}/rest/v1/edit_requests?customer_id=eq.${encodeURIComponent(resolved.customerId)}&status=eq.ready_for_review&select=id,description,request_type,ai_edit_summary,created_at,website_id&order=created_at.desc`,
      {
        headers: {
          'apikey': supabaseKey,
