-- Let customer-provided intake/catalog content qualify a business for website
-- generation even when no matching Google listing/enrichment data exists.
...
AS $$
  ...
  WHERE NULLIF(btrim(b.name), '') IS NOT NULL
    ...
    AND (
      ...
      OR NULLIF(btrim(COALESCE(b.description, '')), '') IS NOT NULL
      OR NULLIF(btrim(COALESCE(b.notes, '')), '') IS NOT NULL
      OR NULLIF(btrim(COALESCE(b.founder_description, '')), '') IS NOT NULL
      OR EXISTS (
        SELECT 1
