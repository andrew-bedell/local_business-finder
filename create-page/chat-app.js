...
async function transcribeVoiceBlob(blob) {
  const res = await fetch('/api/public-builder/transcribe-voice', {
    method: 'POST',
...
function App() {
  ...
  useEffect(() => {
    return () => {
      cleanupVoiceSession(voiceRef.current);
  ...
  async function startVoiceRecording() {
    if (typing || inputOff || voiceState !== 'idle') return;
    setVoiceError('');

    const AudioContextClass = window.AudioContext || window.webkitAudioContext;
    if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia || !AudioContextClass) {
      setVoiceError('Tu navegador no permite grabar audio aqui. Escribe tu respuesta o prueba en Chrome/Safari actualizado.');
      return;
    }
    ...
    try {
      ...
      voiceRef.current = null;
      setVoiceState('idle');
      setVoiceError('No pudimos abrir el microfono. Revisa el permiso del navegador e intenta de nuevo.');
    }
  }
  ...
  async function stopVoiceRecording() {
    ...
    try {
      ...
      if (duration < 500 || !samples.length) {
        throw new Error('No se escucho suficiente audio. Intenta grabar otra vez.');
      }

      const wavBlob = encodeWav(samples, session.sampleRate);
      const transcript = await transcribeVoiceBlob(wavBlob);
      if (!transcript) {
        throw new Error('No se detecto voz clara. Intenta grabar de nuevo.');
      }
      ...
      setVoiceError(error && error.message ? error.message : 'No se pudo transcribir el audio. Intenta de nuevo.');
    } finally {
      setVoiceState('idle');
    }
  }
  ...
  const canSendText = inputVal.trim() && !typing && voiceState === 'idle';
  const inputPlaceholder = voiceState === 'recording'
    ? 'Escuchando... toca el microfono para terminar'
  ...
  return (
    <div style={outerStyle}>
      <div style={frameStyle}>
        ...
        <div style={{ background: C.waHeader, padding: '8px 10px', flexShrink: 0, borderTop: '1px solid rgba(255,255,255,.04)' }}>
          {inputOff ? (
            <div style={{ flex: 1, textAlign: 'center', fontSize: 12, color: C.waSub, padding: '10px 0' }}>Procesando...</div>
          ) : (
            <>
              {voiceError ? (
                <div style={{ color: '#FCA5A5', fontSize: 11.5, lineHeight: 1.35, padding: '0 4px 7px', textAlign: 'center' }}>
                  {voiceError}
                </div>
              ) : null}
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <div style={{ flex: 1, background: 'rgba(255,255,255,.08)', borderRadius: 24, padding: '10px 14px', display: 'flex', alignItems: 'center', minWidth: 0 }}>
                  <input
                    ref={inputRef}
                    value={inputVal}
                    ...
                    onKeyDown={event => {
                      if (event.key === 'Enter' && !event.shiftKey) {
                        ...
                        if (canSendText) handleSend();
                      }
                    }}
                    placeholder={inputPlaceholder}
                    style={{ flex: 1, minWidth: 0, background: 'none', border: 'none', outline: 'none', color: C.waText, fontSize: 16, fontFamily: 'DM Sans, sans-serif' }}
                  />
                </div>
                <button
                  type="button"
                  ...
                  disabled={typing || voiceState === 'transcribing'}
                  title={micTitle}
                  aria-label={micTitle}
                  style={{
                    width: 42,
                    ...
                    borderRadius: '50%',
                    border: 'none',
                    cursor: typing || voiceState === 'transcribing' ? 'not-allowed' : 'pointer',
                    flexShrink: 0,
                    background: voiceState === 'recording' ? C.red : voiceState === 'transcribing' ? 'rgba(255,255,255,.08)' : 'rgba(255,255,255,.08)',
                    display: 'flex',
                    alignItems: 'center',
                    justifyContent: 'center',
                    transition: 'background .2s, transform .2s',
                    animation: voiceState === 'recording' ? 'glow 1s ease-in-out infinite' : 'none',
                    opacity: typing ? 0.55 : 1
                  }}
                >
                  {voiceState === 'transcribing' ? (
                    <span style={{ width: 18, height: 18, borderRadius: '50%', border: '2px solid rgba(255,255,255,.35)', borderTopColor: '#fff', animation: 'spin .8s linear infinite' }} />
                  ) : voiceState === 'recording' ? (
                    <svg width="17" height="17" viewBox="0 0 17 17" fill="none" aria-hidden="true"><rect x="4" y="4" width="9" height="9" rx="2" fill="white" /></svg>
                  ) : (
                    <svg width="18" height="18" viewBox="0 0 18 18" fill="none" aria-hidden="true"><path d="M9 11.5C10.66 11.5 12 10.16 12 8.5V4.5C12 2.84 10.66 1.5 9 1.5C7.34 1.5 6 2.84 6 4.5V8.5C6 10.16 7.34 11.5 9 11.5Z" stroke="white" strokeWidth="1.7" strokeLinecap="round" /><path d="M3.75 8.25C3.75 11.15 6.1 13.5 9 13.5M9 13.5C11.9 13.5 14.25 11.15 14.25 8.25M9 13.5V16" stroke="white" strokeWidth="1.7" strokeLinecap="round" /></svg>
                  )}
                </button>
                <button
                  onClick={() => handleSend()}
                  ...
                  style={{
                    ...
                    flexShrink: 0,
                    background: canSendText ? C.green : 'rgba(255,255,255,.08)',
                    display: 'flex',
                    alignItems: 'center',
                    justifyContent: 'center',
                    transition: 'background .2s',
                    opacity: voiceBusy && !canSendText ? 0.55 : 1
                  }}
                >
                  <svg width="18" height="18" viewBox="0 0 18 18" fill="none" aria-hidden="true"><path d="M2 9L16 2L9 16L7.5 10.5L2 9Z" fill="white" /></svg>
                </button>
              </div>
            </>
          )}
